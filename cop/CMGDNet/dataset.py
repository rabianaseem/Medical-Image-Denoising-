...
class ImageDataTrain(data.Dataset):
    def __init__(self, data_root, data_list,image_size):
        ...
        with open(self.sal_source, 'r') as f:
            self.sal_list = [x.strip() for x in f.readlines()]           #print(len(sal_list))
    ...
    def __getitem__(self, item):
        ...
        #print(len(sal_list))
        sal_image = load_image(os.path.join(self.sal_root, self.sal_list[item].split()[0]), self.image_size)
        sal_depth = load_image(os.path.join(self.sal_root, self.sal_list[item].split()[1]), self.image_size)
        sal_label = load_sal_label(os.path.join(self.sal_root, self.sal_list[item].split()[2]), self.image_size)
...
def load_image(path,image_size):
    ...
    im1 = nib.load(path)  	                       #.get_data()     #cv2.imread(path)    
    im = im1.get_fdata()
...
def load_sal_label(path,image_size):
    if not os.path.exists(path):
        print('File {} not exists'.format(path))
    im1 =  nib.load(path)                 #cv2.imread(path, cv2.IMREAD_GRAYSCALE)
    im = im1.get_fdata()
    in_ =  np.array(im, dtype=np.float32)        #np.array(im, dtype=np.float32)
    in_ = cv2.resize(in_, (image_size, image_size))
    
    #in_ = cv2.resize(in_, (image_size, image_size))
    in_ = Normalization2(in_)       #  in_ = Normalization(in_)
...
def Normalization(image):
    in_ = image[:, :, ::-1]
    in_ = in_ /3000.0  #in_ / 1000.0          # in_ = in_ / 255.0
    in_ -= np.array((0.485, 0.456, 0.406))
    in_ /= np.array((0.229, 0.224, 0.225))
    return in_

def Normalization2(image):
    in_ = image #  image[:, :, ::-1]
    in_ = in_ /3000.0     #in_ / 1000.0          # in_ = in_ / 255.0
    in_ -= np.array((0.485, 0.456, 0.406))
    in_ /= np.array((0.229, 0.224, 0.225))
