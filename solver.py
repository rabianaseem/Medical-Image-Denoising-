...
class Solver(object):
    ...
    def build_model(self):
        ...
        if torch.cuda.device_count() > 1:        
            ...
            #self.net = torch.nn.DataParallel(net, device)            # net = torch.nn.DataParallel(net, device)
        ...
        for epoch in range(self.config.epoch):
            ...
            for i, data_batch in enumerate(self.train_loader):
                ...
                if self.config.cuda:
                    ...
                    #sal_image = torch.nn.DataParallel(sal_image, device)
                    #sal_depth = torch.nn.DataParallel(sal_depth, device_id)

                    #sal_label = torch.nn.DataParallel(sal_label, device_id)
                ...
                #sal_loss_fuse = sal_loss_final + 256 * sal_loss_coarse + 256 * sal_loss_mid
                sal_loss_fuse = sal_loss_final + 1000 * sal_loss_coarse + 1000 * sal_loss_mid


             

                # ssim_score = ssim(sal_label, sal_final, data_range=1000)  #  ssim_score = ssim(sal_label, sal_final, data_range=1000, size_average=False) # return (N,)
...
'''
              ...
              #   #ssim_score = pytorch_msssim.ssim(sal_label, sal_final)
